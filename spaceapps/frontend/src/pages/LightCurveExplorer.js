  ...
  const runPrediction = async () => {
    ...
    try {
      ...
      try {
        ...
        const explanationResponse = await fetch(`${apiUrl}/ai/explain`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (explanationResponse.ok) {
          ...
          setLlmResponse(explanationData.explanation);
        } else {
          throw new Error('Backend AI explanation failed');
        }
      } catch (aiError) {
        console.error('Backend AI error:', aiError);
        // Fallback to basic response if backend AI fails
...
Would you like to explore specific aspects of this classification or ask questions about the analysis methodology?`;
  ...
  const sendChatMessage = async () => {
    ...
    try {
      // Use backend AI for contextual responses
      const apiUrl = process.env.REACT_APP_API_URL || 'http://localhost:8000';
      const chatResponse = await fetch(`${apiUrl}/ai/chat`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          prediction: {
            ...
            probabilities: modelPrediction.probability_scores
          },
          model_inputs: modelInputs
        })
      });
